const Engine = Matter.Engine;
const World= Matter.World;
const Bodies = Matter.Bodies;
const Constraint=Matter.Constraint 

var engine, world;
var box1, pig1, log1;
var bg=("angrybirdsnight.jpg"),bgImage=0
var score=0, birdsound, pigsound
var birds= []
var bgimg
function preload() {
getbackgroundimage()  
birdsound=loadSound("bird_flying.mp3")
pigsound=loadSound("pig_snort.mp3")
bgimg=loadImage(bg)
}
function setup(){
    var canvas = createCanvas(1200,400);
    engine = Engine.create();
    world = engine.world;

    box1 = new Box(700,320,70,70);
    box2 = new Box(920,320,70,70);
    ground = new Ground(600,height,1200,20)
    platform=new Ground(150,305,300,170)
    pig1=new Pig(810,350)
    log1=new Log(810,260,300,PI/2)
    box3=new Box(700,240,70,70);
    box4=new Box(920,240,70,70);
    box5=new Box(810,160,70,70);
    pig2=new Pig(810,220)
    log2=new Log(810,180,300,PI/2)
    log3=new Log(760,120,150,+PI/7)
    log4=new Log(870,120,150,-PI/7)
    bird=new Bird(200,50);
    bird2=new Bird(150,170);
    bird3=new Bird(100,170);
    bird4=new Bird(50,170);
birds.push(bird4)
birds.push(bird3)
birds.push(bird2)
birds.push(bird)

    sling1=new Slingshot(bird.body,{x:200,y:50});
}

function draw(){
    if(bgImage){
    
   
    background(bgImage);
   
    textSize(25)
    text("Score:"+score,width-300,50)
    if(birds.length>=0){
        textSize(15)
        text("Press space to load the next bird",width/2-100,60)
    }
    else{
        text("Reload, to reload the game", width/2-200,70)
    }
}
else{
    
    
   
        background(bgimg);
       
        textSize(25)
        text("Score:"+score,width-300,50)
        if(birds.length>=0){
            textSize(15)
            text("Press space to load the next bird",width/2-100,60)
        }
        else{
            text("Reload, to reload the game", width/2-200,70)
        }
    }  

    Engine.update(engine);
   
    box1.display();
    box2.display();
    box3.display();
    box4.display();
    box5.display();
    pig1.display();
    pig1.Score()
    pig2.display();
    pig2.Score()
    ground.display();
    log1.display();
    log2.display();
    log3.display();
    log4.display();
     bird.display();
    bird2.display();
    bird3.display();
    bird4.display();
    
    
    
    
     sling1.display();
    platform.display();
}
function mouseDragged() {
Matter.Body.setPosition(birds[birds.length-1].body,{x:mouseX,y:mouseY})
Matter.Body.applyForce(birds[birds.length-1].body,birds[birds.length-1].body.position,{x:5,y:-5})

}

function mouseReleased() {
sling1.fly()
birdsound.play()
birds.pop()
}
function keyPressed(){
 if (keyCode==32){
     if(birds.length>=0){
        sling1.attach(birds[birds.length-1].body)
     }
    
 }   
}
async function getbackgroundimage(){
    var response=await fetch("http://worldtimeapi.org/api/timezone/America/New_York")
    var responseJSON=await response.json()
    var datetime= responseJSON.datetime
    var hour= datetime.slice(11,13)
    if (hour>=06&&hour<=19){
        bg="bg.png"
    } else{
        bg="angrybirdsnight.jpg"
    }
    bgImage=loadImage(bg)
}
